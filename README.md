...
- Codex 的 `Responses` 与 Claude Code 的 Anthropic Messages 原生直连上游；其余协议组合由受限的本机回环网关转换。客户端配置只得到本机端点和能力令牌，上游地址与密钥不会进入客户端配置、预览、状态或日志。
- Codex 跨协议路由会在候选配置中把 `web_search` 固定为 `disabled`，但不会改写保存的通用偏好；回到 `Responses` 后原值恢复。`client_metadata`、`prompt_cache_key`、`reasoning.summary=auto` 与 `reasoning.encrypted_content` 不会转发到非 Responses 上游，变更预览会明确提示这一限制。
...
- 可选择把应用自己的配置备份到你控制的 Supabase 项目；云端内容在离开设备前加密，快照包含供应商的 API 格式和最大输出 token，不包含原始客户端配置文件；认证请求头始终由保存的 API 格式推导。恢复旧快照时，应用会一次性升级并重新加密回写云端。

### 扩展（Skills 与 MCP）有自己的工作区

- 「扩展」页集中发现、安装和管理 Skill 与 MCP 服务：先入库，再按客户端与作用域（用户级、项目共享、Claude 项目私有）多目标绑定部署；Skills 支持多选批量检查更新并合成一份统一预览，Skill 绑定可固定内容版本；供应商切换不会触碰已部署的扩展。
...
- 在「网关」页可以直接修改端口：先校验并占用新端口，再预览每个客户端的完整服务地址变化，确认后在一笔可恢复事务中只更新网关端点与网关端口；上游地址、API 密钥、模型、通用设置与本机能力令牌保持不变。关闭预览会立即释放端口，未确认的预览也会自动过期。修改完成后请重启相关客户端或会话。
...
2. 在档案中选择 API 格式，认证请求头会随格式自动确定；再填写要使用的模型与连接信息，并按需要调整通用设置。
