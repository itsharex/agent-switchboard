// Rebuild the pinned offline data from an explicitly supplied clean CC Switch checkout.
...
if (!checkout) throw new Error("Usage: node scripts/import-codex-presets.mjs <cc-switch-checkout>");
const source = "src/config/codexProviderPresets.ts";
const head = execFileSync("git", ["-C", checkout, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (head !== revision) throw new Error("CC Switch preset reference revision does not match the pinned baseline");
