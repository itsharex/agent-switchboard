...
fn prompt_cache_auto_is_narrow_and_explicit_keys_win_over_real_sessions() {
    for (endpoint, enabled) in [
        ...
        apply(
            ...
            endpoint,
            &Default::default(),
            &json!({"prompt_cache_key":"explicit"}),
            &mut wire,
            Some("real-session"),
        )
        .unwrap();
        assert_eq!(
            wire.get("prompt_cache_key"),
            enabled.then_some(&json!("explicit")),
            "{endpoint}"
        );
    }
...
fn disabled_cache_routing_wins_and_provider_overrides_remain_explicit() {
    ...
    let options = CodexRequestOptions {
        prompt_cache_routing: CodexPromptCacheRouting::Disabled,
        ..Default::default()
    };
    apply(
        UpstreamProtocol::ChatCompletions,
...
fn moonshot_schema_rewrite_preserves_literal_data_and_existing_constraints() {
    ...
    assert_eq!(
        result["properties"]["text"]["allOf"],
        json!([{"$ref":"#/$defs/label"},{"minLength":2}])
    );
    assert_eq!(
        result["properties"]["text"]["default"],
        schema["properties"]["text"]["default"]
    );
    ...
    apply(
        UpstreamProtocol::ChatCompletions,
        "https://moonshot.cn.evil.test/v1",
        &Default::default(),
        &json!({}),
        &mut other,
        None,
...
fn anthropic_compatibility_is_opt_in_and_one_m_is_request_specific() {
    ...
    .unwrap();
    assert_eq!(ordinary["model"], "claude");
    assert_eq!(beta.as_deref(), Some("context-1m-2025-08-07"));
    assert_eq!(ordinary["system"], "Original Codex instructions");
    assert!(!ordinary.to_string().contains("cache_control"));
    let options = CodexRequestOptions {
...
#[test]
fn anthropic_cache_places_four_breakpoints_with_a_prior_user_anchor() {
    let options = CodexRequestOptions {
        anthropic_cache_ttl: Some(CodexAnthropicCacheTtl::FiveMinutes),
        ..Default::default()
    };
    ...
    .unwrap();
    // tools + system + newest message; the prior-user anchor needs a long turn.
    assert_eq!(body.to_string().matches("cache_control").count(), 3);
    assert_eq!(
        body["messages"][2]["content"][0]["cache_control"]["ttl"],
        "5m"
    );
    // Long tool loop: newest breakpoint on the trailing tool_result, second
    // user anchor on the turn-opening message.
    let mut loop_body = json!({"model":"claude","system":"system",
    ...
    .unwrap();
    assert_eq!(
        loop_body.to_string().matches("cache_control").count(),
        4,
        "tools + system + newest + prior-user anchor"
    );
    assert_eq!(
        loop_body["messages"][4]["content"][0]["cache_control"]["ttl"],
        "5m"
    );
    assert_eq!(
        loop_body["messages"][2]["content"][0]["cache_control"]["ttl"],
        "5m"
    );
    assert!(loop_body["messages"][0]["content"][0]
        .get("cache_control")
        .is_none());
    assert!(loop_body["messages"][1]["content"][0]
        .get("cache_control")
        .is_none());
    assert!(loop_body["messages"][3]["content"][0]
        .get("cache_control")
        .is_none());
