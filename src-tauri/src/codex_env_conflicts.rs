//! Codex 环境变量冲突（Codex 专属）：系统环境（Windows 注册表 HKCU/HKLM 的
//! `Environment` 键）或 shell 启动文件里的 `OPENAI*` 变量会盖过 `config.toml` /
//! `auth.json` 中的供应商路由与凭据（Codex CLI 读取 `OPENAI_API_KEY`、
//! `OPENAI_BASE_URL` 等）。扫描只读；删除前把完整值备份到应用状态目录，恢复只接受
//! 该目录下的备份。返回给界面的值经统一脱敏。
...
const BACKUP_DIR: &str = "backups/codex-env";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub(crate) struct CodexEnvConflict {
    pub var_name: String,
    /// Secret-shaped values are redacted; only the backup keeps the real value.
    pub value_preview: String,
    pub source: CodexEnvSource,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", tag = "kind")]
pub(crate) enum CodexEnvSource {
...
impl Entry {
    fn conflict(&self) -> CodexEnvConflict {
        CodexEnvConflict {
...
pub(crate) struct CodexEnvScan {
    pub conflicts: Vec<CodexEnvConflict>,
...
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub(crate) struct CodexEnvSelection {
    pub var_name: String,
    pub source: CodexEnvSource,
}
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct CodexEnvBackup {
    pub file_name: String,
    pub created_at: String,
    pub entries: Vec<CodexEnvConflict>,
}
...
pub(crate) fn scan() -> Result<CodexEnvScan, String> {
    let entries = collect()?;
    Ok(CodexEnvScan {
...
pub(crate) fn remove(
    root: &Path,
    selections: &[CodexEnvSelection],
    expected_revision: &str,
) -> Result<CodexEnvBackup, String> {
...
fn write_backup(root: &Path, entries: &[Entry]) -> Result<CodexEnvBackup, String> {
...
fn delete_entry(entry: &Entry) -> Result<(), String> {
    match &entry.source {
        CodexEnvSource::File { path, line } => {
            remove_export_line(Path::new(path), *line, &entry.var_name)
        }
        CodexEnvSource::System { location } => platform::delete_system(location, &entry.var_name),
    }
}
...
pub(crate) fn list_backups(root: &Path) -> Result<Vec<CodexEnvBackup>, String> {
...
pub(crate) fn restore(root: &Path, file_name: &str) -> Result<usize, String> {
    ...
    for entry in &file.entries {
        match &entry.source {
            CodexEnvSource::File { path, .. } => {
                append_export(Path::new(path), &entry.var_name, &entry.value)?
            }
            CodexEnvSource::System { location } => {
...
#[cfg(windows)]
mod platform {
    use super::{matches, CodexEnvSource, Entry};
    ...
    pub(super) fn can_remove(_source: &CodexEnvSource) -> bool {
        true
    }
...
mod platform {
    ...
    pub(super) fn can_remove(source: &CodexEnvSource) -> bool {
        matches!(source, CodexEnvSource::File { .. })
    }
