...
use super::document::{json_or_toml_step, read_document};
...
impl Planner<'_> {
    pub(super) fn clear_disable_steps(
    ...
    ) -> Result<(Vec<PlanStep>, Option<ManagedBaselineFile>, Vec<String>), CommandError> {
        ...
        let mut document_texts: BTreeMap<String, String> = BTreeMap::new();
        let mut initial_hashes: BTreeMap<String, String> = BTreeMap::new();
        ...
        for (target_path, entry_pointer, original_value, last_document_hash) in entries {
            let initial_hash = match initial_hashes.get(&target_path) {
                Some(hash) => hash.clone(),
                None => {
                    let document = PathBuf::from(&target_path);
                    let Some(text) = read_document(&document)? else {
                    ...
                    document_texts.insert(target_path.clone(), text);
                    initial_hashes.insert(target_path.clone(), hash.clone());
                    hash
                }
            };
            if initial_hash != last_document_hash {
                warnings.push(format!(
                    "{} 已在本应用上次写入后发生外部变更，未恢复原始 Skill 停用规则",
                    PathBuf::from(&target_path).display()
                ));
                continue;
            }
            let text = document_texts
                .get(&target_path)
                .cloned()
                .ok_or_else(|| CommandError::new("extension-baseline", "Skill 基线文档缺失"))?;
            let (rendered, restored) = match client {
                AppKind::Codex => {
                    ...
                    let (rendered, changes) =
                        apply_codex_skill_rule_restore(&text, rule_path, original_value.as_deref())
                ...
                AppKind::Claude => {
                    ...
                    let (rendered, changes) =
                        apply_claude_skill_override_restore(&text, name, original_value.as_deref())
                            .map_err(adapter_error)?;
                    (rendered, !changes.is_empty())
                }
            };
            if restored {
                document_texts.insert(target_path.clone(), rendered);
                restored_slots.insert((target_path, entry_pointer));
            }
        }

        if restored_slots.is_empty() {
            return Ok((Vec::new(), None, warnings));
        }
        ...
        for target_path in restored_slots
            .iter()
            .map(|(target_path, _)| target_path)
            .collect::<BTreeSet<_>>()
        {
            let rendered = document_texts
                .get(target_path)
                .cloned()
                .ok_or_else(|| CommandError::new("extension-baseline", "Skill 基线文档缺失"))?;
            steps.push(json_or_toml_step(
                client,
        ...
        Ok((steps, Some(cleared), warnings))
    }

    /// Re-enables a Claude private-project MCP in one render. The private
    /// server and its disabledMcpServers member share the user document, so
    /// emitting separate document writes would make the second precondition
    /// stale as soon as the first one succeeds.
