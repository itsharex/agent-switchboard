...
export function GatewayTelemetry({
  ...
  return (
    <div className="asb-gateway-telemetry">
...
function buildProtocolUsage(samples: GatewaySample[]) {
  ...
  for (const sample of samples) {
    if (sample.upstreamProtocol !== null) {
      counts.set(
        sample.upstreamProtocol,
        (counts.get(sample.upstreamProtocol) ?? 0) + 1,
      );
    }
  }
...
function recentColumns(
  profileNames: Map<string, string>,
): Array<TableColumn<GatewaySample>> {
  return [
    {
      key: "time",
      header: "时间",
      render: (sample) => (
        <span className="asb-gateway-row-value asb-code">
          {formatClock(sample.atMs)}
        </span>
      ),
    },
    ...
    },
    {
      ...
      render: (sample) =>
        sample.routeRevision === null ? (
          ...
          <span className="asb-gateway-revision asb-code">
            {sample.routeRevision}
          </span>
        ),
    },
    {
    ...
    },
    {
      ...
      render: (sample) => {
        ...
        if (sample.status >= 400) {
          return <span className="asb-fail-text">HTTP {sample.status}</span>;
        }
    ...
    },
    {
      ...
      header: "耗时",
      render: (sample) => (
        <span className="asb-gateway-row-value">
...
function GatewayRequestPath({ sample }: { sample: GatewaySample }) {
  ...
  const translated = isProtocolTranslation(
    sample.clientProtocol,
    sample.upstreamProtocol,
  );
  return (
    <span className="asb-gateway-request-path">
      ...
      <span
        className={`asb-gateway-request-mode is-${translated ? "translation" : "relay"}`}
      >
