...
/** Read-only scan of CC Switch's MCP table, then one confirmed import into the
...
export function McpSource({ operations: op }: { operations: ClaudeOperations }) {
  ...
  return <section aria-label="导入 CC Switch MCP 服务" className="asb-provider-section-fields">
    <h3 className="asb-section-title">从 CC Switch 导入 MCP 服务</h3>
    <p className="asb-scope-note">只读取来源的 MCP 定义写入扩展库；来源中的「已为 Claude 启用」只作提示，部署到 Claude 仍在扩展工作区预览并确认。</p>
    <label className="asb-field"><span>CC Switch MCP 数据库路径</span><Input value={path} disabled={disabled} placeholder="~/.cc-switch/cc-switch.db" onChange={(e) => { setPath(e.target.value); setSource(null); setSelected([]); }} /></label>
    <Button variant="secondary" disabled={disabled || !path.trim()} onClick={() => void op.run(async () => { setSelected([]); setSource(await api.scanClaudeMcpSource(path.trim())); })}>只读扫描 CC Switch MCP 服务</Button>
