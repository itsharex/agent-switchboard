...
import { EnvConflicts } from "./EnvConflicts";
...
export function IntegrationPane({ operations: op }: { operations: ClaudeOperations }) {
  ...
  const plan = (flag: api.ClaudeIntegrationFlag, enable: boolean) => void run(async () => setPreview(await api.previewClaudeIntegration(flag, enable)));
  return <div className="asb-provider-section-fields">
    <p className="asb-scope-note">仅改写 Claude 客户端文件中的单个键，其余内容原样保留；备份与配置切换备份分开保存。</p>
    {view && <>
      <Checkbox label="切换 Claude 供应商后自动同步插件 API Key 标记" checked={view.policy.pluginIntegration} disabled={busy || !!preview}
        onChange={(pluginIntegration) => void run(async () => { setView(await api.setClaudeIntegrationPolicy({ pluginIntegration }, true)); changed("Claude 客户端集成策略已保存。"); })} />
      {view.flags.map((flag) => <section key={flag.flag} aria-label={FLAGS[flag.flag].title} className="asb-provider-section-fields">
        <h3 className="asb-section-title">{FLAGS[flag.flag].title}</h3>
    ...
    {preview && <section aria-label="Claude 客户端集成预览" className="asb-provider-section-fields">
      ...
      <div className="asb-form-actions">
        ...
        {preview.changes.length > 0 && <Button variant="primary" disabled={busy} onClick={() => void run(async () => {
          setView(await api.applyClaudeIntegration(preview, true)); setPreview(null); changed("Claude 客户端集成标记已写入并备份。");
        })}>确认写入 Claude 客户端标记</Button>}
      </div>
    </section>}
    <Button variant="secondary" disabled={busy || !!preview} onClick={() => void run(async () => setView(await api.getClaudeIntegration()))}>重新读取 Claude 客户端集成状态</Button>
    <EnvConflicts operations={op} />
    <McpSource operations={op} />
  </div>;
}
