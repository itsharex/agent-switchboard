...
describe("ExtensionsPage", () => {
  ...
  it("previews and confirms a takeover of a discovered native MCP", async () => {
    ...
    const dialog = await screen.findByRole("dialog", { name: "接管 native-docs" });
    // The preview shows the typed scope and the restore promise, never a path.
    expect(within(dialog).getByText(/Codex 用户级配置$/)).toBeInTheDocument();
    ...
    await user.click(within(dialog).getByRole("button", { name: "确认接管" }));
