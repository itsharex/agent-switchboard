...
describe("BackupsPage", () => {
  it("separates local history and encrypted cloud backup into two tabs", async () => {
    ...
    expect(localTab).toHaveAttribute("aria-controls", "backup-local-panel");
    expect(cloudTab).toHaveAttribute("aria-controls", "backup-cloud-panel");
    ...
    const cloudPanel = document.getElementById("backup-cloud-panel")!;
    expect(cloudPanel).toHaveAttribute("role", "tabpanel");
    expect(cloudPanel).toHaveAttribute("aria-labelledby", cloudTab.id);
