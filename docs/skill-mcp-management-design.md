...
| `src-tauri/src/config_store/{mod,snapshot}.rs` | 重置会删除整个 `state/configuration/`，快照只含供应商、通用配置、历史 | 扩展放独立根目录，现有重置和云备份契约不扩大 |
...
扩展                              [Skills] [MCP]
[全部客户端 v] [用户 / 选择项目 v] [搜索              ]
[已管理] [本机发现] [添加来源/服务]

名称              来源/类型       Codex      Claude      操作
接口规范          本地 Skill      已部署      已停用      查看
文档检索          HTTP            待应用      —           查看

选中项详情：说明 / 文件或连接参数 / 应用目标 / 最近结果
变更预览：本次目标、增删改、冲突、凭据去向、重载提示
                                                [取消] [确认应用]
```

这是一份本产品的信息结构草图，不复刻参考项目的页面或动作流程。页面沿用 `Frosted Relay` 的单一工作区、高对比度内容面、真实 DOM 和现有控件；不新增仪表盘指标、横幅或常驻路由卡。

「已管理 / 本机发现」控制数据集合；Skills 的来源浏览在「添加来源」中展开。MCP 的「添加」菜单提供表单新建、粘贴配置、从本机导入。这样可把尚未入库的发现结果与已应用的条目分开。

### 5.2 行状态与交互

- 列表显示摘要，选中后在下方展开详情；长文件树和配置差异在详情内部滚动。
- 客户端单元格显示实际部署状态；点击「管理应用目标」修改草稿。选择目标用 `Checkbox`，不使用立即亮起、让人误以为已生效的 `Switch`。
- 编辑已有资源点击「保存到库」只更新本地定义；已部署目标显示「有待应用变更」。首次「安装到客户端」可在一个事务中同时完成入库和部署，不要求用户重复保存。
- 操作预览在详情下方展开。最终确认沿用已有 `ConfirmSheet`，取消不改客户端；应用期间展示逐目标进度，取消仅在开始替换前可用。
- 搜索输入不发起远端请求。远端查询使用独立「搜索来源」按钮并显示来源域和缓存时间。
- 空状态给出「从本机发现」及「添加」；读取失败保留上次结果并标示时间，不把失败显示为零条扩展。
- 导航拥挤时使用现有导航体系的可访问溢出菜单；不压缩窗口按钮或把点击目标缩到 40px 以下。980px、1280px、1440px、200% 缩放列为布局验收场景。
- 延续 `tokens.css` 的颜色、间距和动效；表格、菜单、焦点及减少动态效果都沿用现有规范。以文字补充状态色。
...
通用设置只提供跳转，不重复编辑扩展；`features.skill_mcp_dependency_install` 仍由通用设置拥有。供应商适配器和恢复流程继续保留扩展字段。备份页明确区分供应商历史、扩展操作历史和云备份范围，README 不宣称扩展已上云。
