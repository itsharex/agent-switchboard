...
fn gateway_projection_cannot_send_official_credentials_to_remote_hosts() {
    let plan = plan_b();
    for endpoint in ["https://third-party.test/codex/secret/v1", "http://127.0.0.1:47821/v1", "http://user@127.0.0.1:47821/codex/secret/v1", "http://127.0.0.1:47821/codex/secret/v1?other=1"] {
        let invalid = SwitchPlan::through_gateway(plan.profile.clone(), plan.client_settings.clone(), endpoint.into(), String::new());
